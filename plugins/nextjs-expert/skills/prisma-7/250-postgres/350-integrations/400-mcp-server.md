...
Codex is a terminal-based AI tool where you can add MCP server using the `claud mcp add` command for the local MCP server:

```terminal
codex mcp add prisma-local npx prisma mcp
```

or for the remote MCP server:

```terminal
codex mcp add prisma-remote npx mcp-remote https://mcp.prisma.io/mcp
```

Learn more in the [Codex MCP docs](https://docs.anthropic.com/en/docs/agents-and-tools/codex/tutorials#configure-mcp-servers).

### Codex Desktop only

Follow the instructions in the [Codex Desktop MCP docs](https://modelcontextprotocol.io/quickstart/user#2-add-the-filesystem-mcp-server) to create the required configuration file:
...
1. In a new chat, find the **Connect apps** field in the main Codex UI below the input prompt field
...
The Prisma CLI can detect when it is being invoked by popular AI coding agents such as Codex, Gemini CLI, Qwen Code, Cursor, Aider, and Replit.
