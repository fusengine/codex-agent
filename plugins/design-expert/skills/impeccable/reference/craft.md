...
## Build phase — Gemini Design renderer

When `mcp__gemini-design__create_frontend` is available in the runtime, **default to it** for HTML/JSX generation in craft / build / animate / colorize phases. Do NOT generate HTML inline by hand — Gemini Design is the renderer.

Pass design specs as the **7 XML blocks pattern**, sourced from the confirmed shape brief and `design-system.md`:

1. `<aesthetics>` — overall mood, personality, visual tone (pulled from shape brief).
2. `<style_reference>` — declared reference site + 3 specific elements to honor.
3. `<typography>` — approved font pair, type scale, weights, line-heights (never Inter / Roboto / Arial / Open Sans).
4. `<color_system>` — OKLCH tokens (chroma > 0.05 for accents), light + dark variants, semantic mappings.
5. `<spacing>` — base unit, scale, layout grid ratios.
6. `<states>` — hover / focus-visible / active / disabled / loading / empty / error treatments.
7. `<forbidden>` — anti-AI-slop list: no generic gradients, no Lorem Ipsum, no emoji icons, no centered everything, no purple-to-pink, no glass overload, no prohibited fonts.

Iteration: use `mcp__gemini-design__modify_frontend` to refine. Use `mcp__gemini-design__snippet_frontend` for isolated component fragments inside an existing layout.

Fallback: only generate HTML inline when Gemini Design MCP is unavailable in the current session.
