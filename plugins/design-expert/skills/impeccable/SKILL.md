---
name: impeccable
description: Use when the user wants to design, redesign, shape, critique, audit, polish, clarify, distill, harden, optimize, adapt, animate, or colorize a frontend interface. Covers websites, landing pages, dashboards, product UI, components, forms, onboarding, empty states. Handles UX review, hierarchy, accessibility, responsive behavior, theming, typography, layout, color, motion, UX copy, design tokens. Also bolder/quieter redesigns, live browser iteration, ambitious visual effects.
---

Designs and iterates production-grade frontend interfaces. Real working code, committed design choices, exceptional craft.

**Renderer integration (Gemini Design MCP)** — When the `mcp__gemini-design__*` tools are available, use them as the renderer for the build/craft/animate/colorize phases instead of generating HTML inline. Pass design specs (typography, color tokens, spacing, states, forbidden patterns) as the 7 XML blocks pattern documented in `reference/craft.md` to `mcp__gemini-design__create_frontend`. Use `mcp__gemini-design__modify_frontend` for iteration and `mcp__gemini-design__snippet_frontend` for isolated fragments.
